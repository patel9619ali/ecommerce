...
export default function FAQ({ className = {}, count }: FAQProps) {
    ...
    return (
        <section className="bg-[#dbd4d463] dark:bg-[#fffffff0] py-10">
            <div className="container mx-auto">
                <div className={`flex flex-col items-${className.alignMent} lg:gap-2 gap-2 flex-wrap lg:mb-4 mb-4`}>
                    <h2 className={`w-full text-[19px] sm:text-[37px] font-bold relative z-[2] bg-gradient-to-r from-[#053E54] to-[#057C72] bg-clip-text text-transparent mb-3 bg-clip-text lg:text-${className.alignMent} text-center lg:leading-[48px] leading-normal`}>FAQs. Check out the common questions or doubts</h2>
                </div>
                    ...
                    {!searchTerm && (
                        <TabsList className={`flex flex-row lg:flex-col items-stretch *:justify-start border-b-2 border-[#C9C9C9] lg:border-b-0 lg:border-r-2 lg:border-[#C9C9C9] overflow-x-auto lg:overflow-x-visible lg:overflow-y-auto rounded-none`}>
                            {faqTabs.map((tab) => (
                                ...
                                key={tab.id}
                                value={tab.id}
                                className="font-[500] data-[state=active]:font-[700] lg:text-[23px] text-[14px] data-[state=active]:text-[#053E54] text-[#666666] data-[state=active]:bg-transparent relative before:content-[''] before:absolute before:rounded-none before:opacity-0 data-[state=active]:before:opacity-100 before:transition-opacity before:duration-300 before:bottom-0 before:left-1/2 before:-translate-x-1/2 before:w-[100%] before:border-b-[3px] before:border-[#053E54] lg:before:left-0 lg:before:top-1/2 lg:before:-translate-y-1/2 lg:before:translate-x-0 lg:before:h-3/4 lg:before:w-auto lg:before:border-b-0 lg:before:border-l-[3px] lg:mr-3 lg:pr-0 lg:ps-3 mr-3 pr-0 ps-0 hover:text-[#053E54] hover:font-bold"
                                >
                                {tab.label}
                                </TabsTrigger>
                            ))}
                            <p className="lg:flex hidden font-[700] text-[23px] text-[#053E54] uppercase mt-3 mb-2">Looking for more?</p>
                            {!className.modal ? <Link href={`/faq`} className="lg:flex hidden uppercase font-[600] text-[19px] rounded-[56px] bg-[#053E54] px-4 py-2 text-[#fff] w-max">Visit FAQs Page</Link> : 
                            <>
                    ...
                    {searchTerm && (
                        ...
                        if (btn) (btn as HTMLButtonElement).click();}} className="cursor-pointer relative flex lg:flex-row flex-row-reverse items-start w-full before:content-[''] lg:before:w-[33px] before:w-[27px] lg:before:h-[33px] before:h-[27px] before:border-[2px] before:border-[#66666650] before:rounded-[10px] before:flex-shrink-0 before:transition-all before:duration-300 after:content-['+'] after:absolute lg:after:left-[8px] lg:after:top-[12px] lg:after:text-[25px] after:text-[19px] after:text-[#66666650] after:w-[16px] after:text-center after:transition-all after:duration-300 has-[[data-state=open]]:before:bg-[#C8FFFA] has-[[data-state=open]]:before:border-[#057C72] has-[[data-state=open]]:after:content-['−'] has-[[data-state=open]]:after:text-[#057C72] lg:py-4 py-2  after:right-[5px] lg:after:top-[9px] after:top-[7px] text-[19px]">
                                    <div className="lg:ms-5 flex-1 ms-0">
                                      <AccordionTrigger className="text-[#053E54] font-[500] data-[state=open]:font-[700] text-start lg:text-[23px] lg:p-0 pt-0 pb-0 justify-start flex-1 text-[14px] [&>svg]:hidden">
                                          {item.question}
                            ...
                            <div className="flex items-center justify-center gap-4 mt-5 flex-col">
                              {notFoundFAQ}
                              <p className="text-center text-[23px] font-[700] leading-[30px] text-[#053E54]">Sorry We Couldn’t Find That</p>
            ...
            {!searchTerm &&
              faqTabs.map((tab) => (
                  ...
                  <Accordion type="single" collapsible className="w-full">
                    {activeTabData?.items.map((item, index) => (
                      <AccordionItem key={index} value={`item-${index}`} className="border-b-[2px] border-b-[#C9C9C9] last:border-b-0">
                        ...
                        if (btn) (btn as HTMLButtonElement).click();}} className="cursor-pointer relative flex lg:flex-row flex-row-reverse items-start w-full before:content-[''] lg:before:w-[33px] before:w-[27px] lg:before:h-[33px] before:h-[27px] before:border-[2px] before:border-[#66666650] before:rounded-[10px] before:flex-shrink-0 before:transition-all before:duration-300 after:content-['+'] after:absolute lg:after:left-[8px] lg:after:top-[12px] lg:after:text-[25px] after:text-[19px] after:text-[#66666650] after:w-[16px] after:text-center after:transition-all after:duration-300 has-[[data-state=open]]:before:bg-[#C8FFFA] has-[[data-state=open]]:before:border-[#057C72] has-[[data-state=open]]:after:content-['−'] has-[[data-state=open]]:after:text-[#057C72] lg:py-4 py-2  after:right-[5px] lg:after:top-[9px] after:top-[7px] text-[19px]">
                        <div className="lg:ms-5 flex-1 ms-0">
                          <AccordionTrigger className="text-[#053E54] font-[500] data-[state=open]:font-[700] lg:text-[23px] text-start lg:p-0 pt-0 pb-0 justify-start flex-1 text-[14px] [&>svg]:hidden">
                            {item.question}
                          </AccordionTrigger>
                          <AccordionContent className="text-[#666666] font-[400] lg:text-[18px] text-[12px] pb-0 pt-2">
                            {item.answer}
                ...
                <p className="lg:hidden text-center block font-[700] lg:text-[23px] text-[14px] text-[#053E54] uppercase mt-3 mb-2">Looking for more?</p>
                {!className.modal ? <Link href={`/faq`} className="lg:hidden block uppercase font-[600] lg:text-[19px] rounded-[56px] text-[15px] mx-auto bg-[#053E54] px-4 py-2 text-[#fff] w-max">Visit FAQs Page</Link> : ""}
