...
const MobileInformativeSlider = ({ className,productData }: Props) => {
  ...
  const product =
    productData?.find((item) =>
      isPortableJuicerCategory(item.category?.name || item.category?.slug)
    ) || productData?.[0];
